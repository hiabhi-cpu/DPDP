...
| 🟡 | BE/FE | **Front-desk-driven consent flow (Spec B of 2)** — **B1 backend done (2026-07-13); B2 UIs not started.** Consumes Spec A. Reception fires OTP → patient completes at any kiosk by entering the SMS code (the code IS the OTP). **B1 shipped:** notification-service hospital-scoped **OTP-claim** (`SendClaim`/`ResolveClaim`, hashed match within the hospital's active `claimset` — no plaintext codes, secrecy preserved, uniqueness-on-send, per-hospital resolve cap; hospital-JWT `/internal` endpoints); integration-service pending **status** `PENDING→CODE_SENT→DONE` (TTL-preserving set-status, status in masked list); admin-bff **reception role** (least-privilege, server-enforced) + consent-queue proxy + `send-code` orchestration; **migration 0014** adds the `reception` role. Ponytail-trimmed (claim index 3→1 Redis key; dropped supersede + HMAC capture-handle → kiosk reuses the existing capture flow). Live-verified full vertical (reception send-code → resolve → capture linked to `hms_patient_id` → check-by-HMS `allowed:true`); opus review clean. **B2 (design done 2026-07-13, not built):** reception queue **screen** (admin-dashboard, role-scoped, DONE rows hidden = completion-by-disappearance) + **code-only** kiosk (walk-in mobile/OTP steps removed) + kiosk-bff `claim/resolve` proxy (reuses hospital JWT — no service-token) + best-effort `DONE` marking on capture. See `docs/superpowers/{specs/2026-07-13-frontdesk-consent-flow-design.md,specs/2026-07-13-frontdesk-consent-B2-uis-design.md,plans/2026-07-13-frontdesk-consent-B1-backend.md}`. |
