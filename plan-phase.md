...
| Returning-patient dead-end (queue doesn't know who already consented) — added 2026-07-16 | P2 | Found in live testing; hits every repeat visitor, wastes an SMS + a walk, erodes reception's trust in the queue. **Built on `feat/returning-patient-queue` (unmerged).** Blocked on the patient-identity fix: its `/active` lookup keys by MOBILE, so post-merge it would badge a son "already consented" off his mother's row and disable Send code — silently denying him capture. Re-key to `hms_patient_id` = Task 8 of `2026-07-20-patient-identity-key.md`, on that branch, after this merges. |
