...
| ⬜ | mobile | **Kiosk capture retry on flaky network** — retry the capture POST reusing the same `session_id` (server already dedupes via #8). Replaces the original *"kiosk offline mode — WatermelonDB SQLite queue + auto-sync"* row, **dropped 2026-07-15**: Spec B made the kiosk code-only, so it must reach notification-service to resolve the reception code at all (no resolve → no patient identity → nothing to queue), and consent writes are gated on a **live** server-verified OTP session that fails closed (`consent-service/pkg/consent/service/session_client.go`). A queued-offline capture is by definition unverified, and the Redis session TTL expires before reconnect — syncing it later would mean writing `otp_verified=true` on faith, re-opening the hole closed by the ✅➕ "consent writes gated on verified OTP session" row in P1. Idempotency (#8) makes *replays* safe, which is a different problem. **If real offline capture is ever needed** it's not a local queue: notification-service issues a short-lived **signed proof-of-verification** at resolve time that the kiosk stores and replays, so the vault can still prove the OTP happened — and it *still* needs network at resolve time, so it covers the same seconds the retry does. See `docs/superpowers/specs/2026-07-15-kiosk-capture-retry-design.md`. |
