...
| ✅ | BE/FE | **Front-desk-driven consent flow (Spec B of 2)** — **DONE (B1 backend + B2 UIs, 2026-07-13).** Consumes Spec A. Reception fires OTP → patient completes at any kiosk by entering the SMS code (the code IS the OTP). **B1 shipped:** notification-service hospital-scoped **OTP-claim** (`SendClaim`/`ResolveClaim`, hashed match within the hospital's active `claimset` — no plaintext codes, secrecy preserved, uniqueness-on-send, per-hospital resolve cap; hospital-JWT `/internal` endpoints); integration-service pending **status** `PENDING→CODE_SENT→DONE` (TTL-preserving set-status, status in masked list); 
...
| ✅ | BE/FE | **Returning-patient dead-end in the reception queue** — added 2026-07-16, **DONE 2026-07-20**, found in live testing. A patient who **already has an active consent** re-registers → the HMS webhook re-stages them → the queue shows them `PENDING` → reception fires a code → the patient walks to the kiosk, enters it, ticks purposes → **capture 409 `ErrActiveConsentExists`** (correct by design: `consent-service` blocks capture while any purpose is still active). Nobody upstream knows: the queue has **no idea** the patient already consented, so it burns an SMS and sends the patient on a pointless walk. This hits **every repeat visitor** — i.e. most of a real OPD day. Mitigated (not solved) 2026-07-16: the kiosk no longer mislabels the 409 as "code not recognized" and now says "you have already given consent" (commit `6d8c05b`). **Real fix:** the reception queue must know each staged patient's consent state — check by `hms_patient_id` when building the queue, then either hide those rows or badge them "Already consented — no action" and disable Send code. Needs a per-row consent lookup (consent-service `check` is per-purpose, so this likely wants a small "has any active consent" aggregate rather than N×purposes calls). **Shipped 2026-07-20:** the queue now badges already-consented rows (row self-removes after 15s; informational only — a patient changing purposes uses the portal). The lookup is a batch "has any active consent" aggregate as anticipated, keyed by `hms_patient_id`. It was deliberately held back until the patient-identity fix merged: keyed by mobile it would have badged a son off his mother's consent and disabled his Send code, silently denying him capture — worse than the dead-end it fixes. Verified live: two registrations on one mobile, consent as one, only that one badged. |
| ✅ | mobile | **Kiosk capture retry on flaky network** — **done (2026-07-20)**, live-verified. **Replaces** the original *"kiosk offline mode — WatermelonDB SQLite queue + auto-sync"* row, **dropped 2026-07-15**: server-verified OTP means an offline kiosk has nothing it can legitimately capture (full reasoning in the spec). Shipped: 5s `AbortSignal.timeout` on every kiosk request + a 3-attempt retry on the capture POST — in `capture()`, not `post()`, so `resolveClaim` cannot burn its per-hospital cap on a retry. Measured live: a *hung* service (docker pause) gives 3 attempts at 6007/6002ms each = the 5s timeout firing + the 1s gap, 17.0s total; a *refused* one fails in ~2s. Two consent-integrity fixes fell out of review: the purpose checkboxes stayed live during the ~17s window while `onConfirm` had already snapshotted them (a patient unchecking one would see it unchecked while the vault recorded it **granted**), and kiosk-bff cleared the reception queue only on 201 — a replayed capture returns 200, so staff kept chasing patients who had already consented. Exactly one vault row after four capture calls. Specs/plans: `docs/superpowers/{specs,plans}/2026-07-15-kiosk-capture-retry*.md`. |
| ✅ | mobile | **Kiosk error copy — honest and actionable** — **done (2026-07-20)**, live-verified. Found by live outage-testing the row above, not by review: `consentError` gave contradictory advice for one situation — a *hung* consent-service said "try again", a *refused* one said "go to the front desk". Both mean unreachable; staff can fix neither, and pressing Confirm again is idempotent-safe. Now both reuse the retry loop's own `retryable()` predicate, so the copy tracks the retry policy instead of restating it. Also: a 403 says **the code expired** *and* returns the patient to the code step — the consent screen has no code field, so the old message promised a resend the UI could never accept (walk to desk → resend → return to a dead screen → 403 again); and `codeError` got the same transient-first fix, since kiosk-bff returns 502 when notification-service is down and that rendered as "Code not recognized", blaming a good code for an outage. A 500 regression test pins the retry/copy coupling. Proven live against real outages: 403→expired+code step, code-step 502→"try again", capture 502→"try again". **Deliberately not done:** an "or ask the front desk" escalation clause — staff cannot fix a down service; revisit if pilot sites see patients loop. See `docs/superpowers/{specs,plans}/2026-07-20-kiosk-transient-error-copy*.md`. |
| ✅➕ | test | **Kiosk live-stack suite** — **done (2026-07-20)**. `frontend/kiosk/live/` + `npm run test:live`; 5 cases, ~36s. Drives the real PWA against the real services through kiosk-bff, stopping/pausing real containers — only the URL origin is stubbed (jsdom serves from `:3000`), never a response. Exists because **every defect in the two rows above was invisible to unit tests**, which mock `fetch`: a BFF returning 500 where we assumed 502 would pass the whole unit suite and still show a patient the wrong sentence. Proves what mocks structurally cannot — that `AbortSignal.timeout` *fires* on a genuinely hung TCP connection (`docker pause`: 3 attempts, 6005/6002ms apart, 17.1s; without it the kiosk spins forever). `run-live.sh` stages its own patients + codes **immediately** before the run: a claim OTP expires in 3 min (`otpExpiry`), and staging earlier is what made an earlier manual run fail at resolve for unrelated reasons; each patient gets a unique mobile since `patient_key` derives from it and a reused consented one 409s. Outside `npm test` **and** `tsc -b` by design (`tsconfig.app.json` includes only `src/`). **Not CI-runnable as-is** — needs the docker stack and mutates shared containers; same position as the tenant-isolation suite, revisit with the P3 CI row. See `frontend/kiosk/live/README.md`. |
...
| ⬜ | infra | GitHub Actions CI/CD → ECR → ECS; **tenant-isolation suite runs on every deploy** (suite itself now built in Phase 1). Decide here what to do with the **kiosk live-stack suite** (P2, `npm run test:live`): it needs the whole docker stack up and stops/pauses containers, so it cannot run in a plain CI job — either give it a compose-backed stage or leave it a pre-merge manual gate. Same call applies to both suites; make it once. |
...
| Kiosk live-stack suite — ✅ done | P2 | Unit tests mock `fetch`, so they cannot prove a real service emits the status the code expects; both kiosk defects of 2026-07-20 were invisible to them. CI wiring deferred to the P3 row. |
| **Patient identity was mobile-only — families collapsed onto one identity** — found 2026-07-20 | P1 | `ComputePatientKey` hashed the mobile alone, but a family shares one number: the 2nd member could never consent (capture 409'd on a relative's row), `check` answered for whichever member consented last, and withdraw revoked the wrong person's consent — the artifact and audit trail named the wrong data principal. Fixed on `fix/patient-identity-key`: identity is now the pair `(patient_key, hms_patient_id)`, and the OTP session binds to the patient so the pairing is server-enforced. Note this also produced 409s that looked exactly like the returning-patient dead-end above. |
