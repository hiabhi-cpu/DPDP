...
| ⬜ | BE/FE | **Front-desk-driven consent flow (Spec B of 2)** — **design done (2026-07-13)**, consumes Spec A. Reception role + "Awaiting consent" queue in admin-dashboard (via admin-bff) → staff fires OTP → patient completes at any kiosk by entering the SMS code. Claim mechanic locked: the code IS the OTP (one code); notification-service gains a hospital-scoped OTP-claim (hashed resolve within the hospital's small active set — no plaintext codes, OTP secrecy preserved) + per-hospital resolve-attempt cap; kiosk-bff stateless via an HMAC-signed capture-handle (mobile never reaches the browser); live queue status PENDING→CODE_SENT→DONE. Build as two plan-phases (B1 backend claim plumbing, B2 the two UIs). See `docs/superpowers/specs/2026-07-13-frontdesk-consent-flow-design.md`. |
